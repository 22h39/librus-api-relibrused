...
class Librus {
  ...
  getSid(login, pass){
    request.get({
        ...
        (err,res,body) => {
        this.authBegin(login, pass, res.headers['set-cookie'][0]);   
      })
  }

  authBegin(login, pass, dzienniksid){
    request.post({
        ...
        (err,res,body) => {
          this.getGrant(dzienniksid);
        })    
  }

  getGrant(dzienniksid){
    request.get({
        ...
        (err,res,body) => {          
          this.getToken(res.headers.location, dzienniksid);
        })
  }

  getToken(token_factory_uri ,dzienniksid){
    request.get({
